...
class PriceListDatabaseService{
  ...
  Stream<List<PriceListModel>> retrieveListStream(){
    return _db.collection('priceList').snapshots().map((QuerySnapshot<Map<String, dynamic>> snapshot){
      return snapshot.docs.map((doc) => PriceListModel.fromDocumentSnapshot(doc)).toList();
    });
  }
